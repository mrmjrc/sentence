...
function themeConfig($form) {
    ...
    $banner = new Typecho_Widget_Helper_Form_Element_Text('banner', NULL, 'back10.png', _t('文章背景图'), _t('填入一个图片url作为默认文章背景图'));
    $form->addInput($banner);
	
    $logo = new Typecho_Widget_Helper_Form_Element_Text('logo', NULL, NULL, _t('LOGO'), _t('填入一个图片url作为logo'));
    $form->addInput($logo);
    
    $links = new Typecho_Widget_Helper_Form_Element_Text('links', NULL, '<li><a target="_blank" href="https://www.murongmengjie.cn/">慕容梦洁</a></li>', _t('友情链接'), _t('填写友情链接（需用li和a标签包住）'));
    $form->addInput($links);
}
?>